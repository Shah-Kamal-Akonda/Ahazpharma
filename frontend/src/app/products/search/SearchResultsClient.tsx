...
const SearchResultsClient: React.FC<SearchResultsClientProps> = ({ initialQuery }) => {
  ...
  // Fetch search results and addresses
  useEffect(() => {
    if (query) {
  ...
  const fetchSearchResults = async (searchQuery: string) => {
    try {
      const response = await axios.get(`${API_URL}/products/search/name?name=${encodeURIComponent(searchQuery)}`);
      setProducts(response.data);
      setErrorMessage(null);
    } catch (err: unknown) {
      if (err instanceof AxiosError) {
        console.error('Error fetching search results:', err.message, err.response?.data);
        setErrorMessage('Failed to fetch search results. Please try again.');
        showToast('Failed to fetch search results. Please try again.', 'error');
      } else {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-b from-blue-50 to-white p-4 sm:p-8 transition-all duration-300 ${isCartOpen ? 'sm:pr-80' : ''} font-poppins`}>
      <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-800 text-center mb-8 sm:mb-10">Search Results for "{query}"</h1>
